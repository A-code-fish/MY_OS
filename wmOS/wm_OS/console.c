...
void cons_newline(struct CONSOLE *cons){
    ...
    if (cons->cur_y < 28 + cmd_y - 16) {
        cons->cur_y += 16;
    } else {
        if (sheet != 0) { 
            for (y = 28; y < 28 + cmd_y - 16; y++) {
                for (x = 8; x < 8 + cmd_x; x++) {
                    sheet->buf[x + y * sheet->bxsize] = sheet->buf[x + (y + 16) * sheet->bxsize];
                }
            }
            for (y = 28 + cmd_y - 16; y < 28 + cmd_y; y++) {
                for (x = 8; x < 8 + cmd_x; x++) {
                    sheet->buf[x + y * sheet->bxsize] = COL8_000000;
                }
            }
            sheet_refresh(sheet, 8, 28, 8 + cmd_x, 28 + cmd_y);
        }
    }
    cons->cur_x = 8;
    return;
...
void cmd_about(struct CONSOLE *cons){
    putfonts8_asc_sht(cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, "wm_OS", 15);
    cons_newline(cons);
    putfonts8_asc_sht(cons->sht, 8, cons->cur_y, COL8_FFFFFF, COL8_000000, "    Edition 13.1.3", 30);
    cons_newline(cons);
}
